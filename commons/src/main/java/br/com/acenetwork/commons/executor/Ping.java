...
public class Ping implements TabExecutor
{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String aliases, String[] args)
	{
		...
		else
		{
			...
			extra[1] = new TextComponent("/" + aliases);
			
			String argsPlayer = bundle.getString("commons.words.player");
			
			if(p == null)
			{
				extra[1].addExtra(" <" + argsPlayer + ">");
			}
			else
			{
				extra[1].addExtra(" [" + argsPlayer + "]");
		...
		if(t == p)
		{
			...
			TextComponent text = Message.getTextComponent("commons.cmd.ping.self", extra);
		...
		else
		{
			...
			TextComponent text = Message.getTextComponent("commons.cmd.ping.other", extra);
			text.setColor(ChatColor.GREEN);
			sender.spigot().sendMessage(text);
		}
